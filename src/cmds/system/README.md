...
- `ctest_cmd.rs` compacts CTest runs into passed/failed/skipped/disabled buckets plus slowest green tests, parses wrapped `***Failed  <reason>` result lines, and keeps each failure's tail capped at `CAP_WARNINGS` lines with tee recovery; explicit verbose/show-only/help/version flags bypass filtering.
